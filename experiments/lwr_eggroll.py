...
def get_lora_update_params_lwr(frozen_noiser_params, base_sigma, iterinfo, param, key):
    ...
    if rank == 0:
        zero_A = jnp.zeros((a, 1), dtype=param.dtype)
        zero_B = jnp.zeros((b, 1), dtype=param.dtype)
    ...
    def init_noiser(
        ...
        **kwargs,
        ...
        if isinstance(rank, dict):
            ...
            if all(v == 0 for v in rank.values()):
                import warnings
                warnings.warn(
                    "LWR-EGGROLL: all layers have rank=0 (no perturbation). "
                    "This means no ES training will occur.",
                    RuntimeWarning)
        elif isinstance(rank, int) and rank == 0:
            import warnings
            warnings.warn(
                "LWR-EGGROLL: scalar rank=0 means no ES training will occur.",
                RuntimeWarning,
            )
