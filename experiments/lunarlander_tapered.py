"""LunarLander-v3 (tapered): testing non-uniform layer architectures.
Architecture: [8, 256, 64, 4] — asymmetric widths, representative of real networks.
Justification: Real networks rarely have uniform layer widths. This tests whether
the sensitivity pilot correctly identifies different rank needs when layer shapes differ.
Optimizer: Adam. Pop: 256, Sigma: 0.05, LR: 0.01. Seeds: 5, Episodes: 5.
"""
...
SEEDS=[0,1,2,3,4]; N_WORKERS=min(cpu_count(),8)
LAYER_SHAPES={"input":(256,8),"hidden":(64,256),"output":(4,64)}
RESULTS_DIR=Path("results/lunarlander_tapered"); RESULTS_DIR.mkdir(parents=True,exist_ok=True)
...
def main():
    ...
    print("="*60);print(f"LUNARLANDER-v3 — TAPERED [8,256,64,4] ({tp:,} params)");print("="*60,flush=True)
    lwr_alloc,alloc_named,ordering=run_rl_pilot(
    ...
    with open(RESULTS_DIR/"summary.json","w") as f:json.dump({"env":ENV_NAME,"architecture":[OBS_DIM]+list(HIDDEN_DIMS)+[ACT_DIM],"note":"tapered - real networks have non-uniform widths","pilot_allocation":alloc_named,"results":all_results},f,indent=2)
