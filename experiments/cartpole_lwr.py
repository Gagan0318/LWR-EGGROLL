"""CartPole-v1: ES method comparison.
Architecture: [4, 256, 256, 2] — matches EGGROLL paper (256 hidden units).
Optimizer: Adam — matches EGGROLL paper.
Population: 256, Sigma: 0.05, LR: 0.01 — consistent with MNIST experiments.
Seeds: 5, Episodes/eval: 5. LWR allocation derived by sensitivity pilot.
"""
...
ENV_NAME="CartPole-v1"; OBS_DIM=4; ACT_DIM=2; HIDDEN_DIMS=(256,256); N_LAYERS=3
POP_SIZE=256; SIGMA=0.05; LR=0.01; MAX_GENS=300; N_EVAL_EPISODES=5
SEEDS=[0,1,2,3,4]; N_WORKERS=min(cpu_count(),8)
LAYER_SHAPES={"input":(256,4),"hidden":(256,256),"output":(2,256)}
RESULTS_DIR=Path("results/cartpole"); RESULTS_DIR.mkdir(parents=True,exist_ok=True)
...
def run_es(seed,method_name,rank_spec=None):
    ...
    try:
        for gen in range(MAX_GENS):
            ...
            if gm>=475: print(f"    SOLVED gen {gen}!",flush=True);break
...
def run_reinforce(seed,lr=0.001,max_episodes=2000,gamma=0.99):
    ...
    for ep in range(max_episodes):
        ...
        if len(recent)>=100 and np.mean(recent[-100:])>=475:print(f"    SOLVED ep {ep}!",flush=True);break
...
def main():
    ...
    print("="*60);print("CARTPOLE-v1 — PILOT-DERIVED LWR (Adam, 256 hidden)");print("="*60,flush=True)
    ...
    for mn,rs in METHODS.items():
        ...
        bests=[r["best_fitness"] for r in mr]
        all_results[mn]={"mean_best":float(np.mean(bests)),"std_best":float(np.std(bests)),"per_seed":bests}
